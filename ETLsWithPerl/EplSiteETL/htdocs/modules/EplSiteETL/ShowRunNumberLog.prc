...
########################################################################
	...
	$MyQuery .= ", TransformationCode, LogMessage";
	$MyQuery .= " FROM ".$globalp->{table_prefix}."_etl_transformations_log";
	$MyQuery .= " WHERE Runnumber = ".$fdat{runnumber};
	$MyQuery .= " ORDER BY DateTime";
	
	my $GetMyQuery = $globalp->{dbh}->prepare ($MyQuery)  or die "Cannot prepare: $MyQuery";
