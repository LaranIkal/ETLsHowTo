error id: file://<WORKSPACE>/Catalogs.sc:local1
...
object script {
  ...
  def main(parameters: Array[String]): Unit = { 
    ...
    val procSequences: List[Long] = if (parameters.isEmpty) {
      List(99.toLong) // default process sequences
    } else {
      parameters.map(_.toLong).toList
    }

    for (procSeq <- procSequences) {
      println(s"Process Sequence to run: $pr@@ocSeq")
    }
  sys.exit(1)
